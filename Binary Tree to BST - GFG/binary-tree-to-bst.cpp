...
private:
    ...
    Node* buildBalancedBST(const vector<int>& sortedValues, int start, int end) {
        ...
        newNode->left = buildBalancedBST(sortedValues, start, mid - 1);
        newNode->right = buildBalancedBST(sortedValues, mid + 1, end);
...
public:
    ...
    Node* binaryTreeToBST(Node* root) {
        if (root == nullptr) {
            return nullptr;  // Handle the case of an empty tree
        }

        vector<int> values;
        inorderTraversal(root, values);

        // Sort the values to create a sorted sequence
